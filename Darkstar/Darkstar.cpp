...
void Darkstar::InitializeWindows(int & screenWidth, int & screenHeight)
{
	...
	if (FULL_SCREEN)
	{
		...
		dmScrenSettings.dmPelsWidth		= (unsigned long)m_screenResolutionX;
		dmScrenSettings.dmPelsHeight	= (unsigned long)m_screenResolutionY;
...
LRESULT Darkstar::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	switch (umsg)
	{
		...
		case WM_LBUTTONDOWN:
		{
			m_Input->MouseKeyDown(MouseKeys::MOUSE_LEFT);
		}
		case WM_LBUTTONUP:
		{
			m_Input->MouseKeyUp(MouseKeys::MOUSE_LEFT);
		}

		case WM_MBUTTONDOWN:
		{
			m_Input->MouseKeyDown(MouseKeys::MOUSE_MIDDLE);
		}
		case WM_MBUTTONUP:
		{
			m_Input->MouseKeyUp(MouseKeys::MOUSE_MIDDLE);
		}

		case WM_RBUTTONDOWN:
		{
			m_Input->MouseKeyDown(MouseKeys::MOUSE_RIGHT);
		}
		case WM_RBUTTONUP:
		{
			m_Input->MouseKeyUp(MouseKeys::MOUSE_RIGHT);
		}
