...
//Pixel Shader
float4 ForwardPlusLightCPUPixelShader(PixelInputType input) : SV_Target
{
    float4 textureColor;
    float3 lightDir;
    ...
    uint2 tileIndex = uint2(floor(input.position.x / BLOCK_SIZE_X), floor(input.position.y / BLOCK_SIZE_Y));
